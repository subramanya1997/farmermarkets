import type { SocialLink, SocialPlatform } from '@/lib/marketDisplay';

/**
 * Brand glyphs as inline paths (24x24 viewBox, fill: currentColor): the
 * project's lucide version dropped brand icons, and shipping an icon library
 * for five glyphs isn't worth it. Shared by the server-rendered summary card
 * and the client explorer card.
 */
...
interface SocialIconLinksProps {
  links: SocialLink[];
  marketName: string;
  /** Client callers use this to fire analytics; the server summary card omits it. */
  onLinkClick?: (link: SocialLink) => void;
}

export function SocialIconLinks({ links, marketName, onLinkClick }: SocialIconLinksProps) {
    ...
    <div className="flex items-center gap-1">
      {links.map((link) => (
        <a
          ...
          aria-label={`${marketName} on ${link.label}`}
          title={link.label}
